...
class SpindleData:
    dataset_path = ""  # 实验中原始数据存放位置
    run_path = ""  # 程序运行的路径,实验结果的保存
    paths = []
    labels = []
    names = []  # 对应的文件名称列表
    ...
    controls_n = 0  # 正常人个数
    length = 0  # 固定长度的设置
    filter_length = 10  # 设置过滤条件，纺锤波的个数小于这个值的病例会被淘汰
    ...
    def __init__(self, dataset_path, run_path, step=0.002, filter_length=10):
    ...
    def get_data_labels(self):  # 返回获取的数据以及标签[0,1,0,1,...]
        dataset_path = self.dataset_path
        cate = [(os.path.join(dataset_path, x)) for x in os.listdir(dataset_path)]
        paths = []
        labels = []
    ...
    def coding_setting(self):  # 所有的数据读取以及存储
        filter_length = self.filter_length
        dataset_path = self.dataset_path
        del_list = []
        sub_cases = 0  # 统计病人删选的个数
        sub_control = 0  # 统计正常人删选的个数
        ## 不同的数据集格式不同，读取方式也不同
        ...
        self.controls_n -= sub_control  # 减去被删选的数
        self.labels = [x for i, x in enumerate(self.labels) if i not in del_list]  # 去除掉对应的标签
        self.names = [x.split("\\")[-1] for i, x in enumerate(self.paths) if i not in del_list]  # 文件名称提取
    ...
    def writer_coding(self):  # 将数据的原始编码写入到文件中（没有对齐的数据）
        ...
        for index, p in enumerate(self.coding_w):
            ...
            name = self.paths[index].split('/')[-1]  # mac下的文件名的提取
...
'''
----------------------------------------------一个独立的模块用来处理亚型---------------------------------------------
处理机制：增加的数据中每一个纺锤波都对应一个亚型，在记录数据的时候用这个亚型来替代原来的编码1
'''


def sub_type_spindle(path):  # "sub_spindle_type"
    # 文件模块的读取
...
if __name__ == '__main__':
    spindle = SpindleData("E:\毕业设计\Spindle-master\datasets\mesa_dataset", "E:\毕业设计\Spindle-master\data\mesa", step=0.001)
