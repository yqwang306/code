...
class BasePreProcessor(ABC):
    ...
    :param data_info: pd.DataFrame(header = ["raw_path", "start_time", "end_time", "gap_time", "name"]),
                        ...
                        or pd.DataFrame(header = ["ID", "CH", "F", "SPINDLE", "Time_of_night", "STOP"]),
                        or pd.DataFrame(header = ["SpindleNo", "Frequency", "Duration", "Amplitude", "Time_of_night"]),
                        the information of every singal
    :param data_type: string, 'bi_class' or 'multi_class'
    """

    data_path = None # the local path of the dataset
    ...
    def __init__(self, data_path, data_name, data_info, data_type):
